...
class PPO(nn.Module):
    def __init__(self):
        ...
        self.buffer = buffer()
        self.policy = Policy()
        self.optimizer_actor = optim.Adam(params=self.policy.actor.parameters(), lr=0.005)
        self.optimizer_critic = optim.Adam(params=self.policy.critic.parameters(), lr=0.005)
    ...
    def update(self):
        ...
        for _ in range(self.epoch):
            ...
            self.loss = -torch.min(surr1, surr2) + 0.5 * self.mse(rollout_reward.detach(), state_value) - 0.1 * entropy
